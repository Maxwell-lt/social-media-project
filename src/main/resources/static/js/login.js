function getValidator(usernameIndex, emailIndex, passwordIndex, passwordFeedbackElement, verifyPasswordIndex = -1) {
	return function (evt) {
		let username = evt.target[usernameIndex].value;
		...
		function validEmail() {
			...
			return email.length <= 254 && email.match(rfc5322);
		}

		function validPassword() {
			let results = zxcvbn(password);
			return results.score > 2 && password.length <= 48;
		}
